...
public interface ChatRoomMembershipRepository extends JpaRepository<ChatRoomMembership, Long> {
    ...
    @Query("select new com.chatServer.chat.dto.response.ChatRoomResponse(crms.chatRoom.id,opponent.member.id, opponent.nickName, opponent.profileImage, c.content, c.readYn,c.member.id, crms2.leftAt, crms.chatRoom.jobPostId) from ChatRoomMembership crms " +
